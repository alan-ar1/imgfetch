...
const (
	...
	esc            = "\x1b"
	st             = "\x1b\\"
	tmuxPassPrefix = esc + "Ptmux;"
	tmuxPassSuffix = esc + "\\"
)

func passthrough(seq string) (string, error) {
	doubled := strings.ReplaceAll(seq, esc, esc+esc)
	return tmuxPassPrefix + doubled + tmuxPassSuffix, nil
}
...
func generateKittyID() (uint32, rgb, byte) {
	id := rand.Uint32()
	maskIndex := byte(id >> 24)
	r := byte(id >> 16)
	g := byte(id >> 8)
	b := byte(id)
	return id, rgb{r: r, g: g, b: b}, maskIndex
}

func encodeImageID(size ImageTermSize, rgb rgb, maskIndex byte) string {
	seq := ""
	for row := range size.Rows {
		seq += fmt.Sprintf("%s[38;2;%d;%d;%dm", esc, rgb.r, rgb.g, rgb.b)
		for col := range size.Columns {
			seq += base + Marks[row] + Marks[col] + Marks[maskIndex]
		}
		if row == size.Rows-1 {
			seq += esc + "[39m"
		} else {
